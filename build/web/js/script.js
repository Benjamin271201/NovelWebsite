...
form.addEventListener('submit', event =>{
        ...
        else if(password.value.length<5 || password.value.length>32 || /\s/.test(username.value)){
            event.preventDefault();
        ...
        if(username.value.length<4 || username.value.length>20 || /\s/.test(username.value)){
            event.preventDefault();
            document.getElementById("usernameMsg").style.visibility = 'visible';
        }
        if(name.value === "" || /\s/.test(username.value)){
             event.preventDefault();
